...
window.Vaadin['_vaadintheme_dostuff_globalCss'] = window.Vaadin['_vaadintheme_dostuff_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_dostuff_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_dostuff_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_dostuff_globalCss'].push(target);
  }
  if (!document['_vaadintheme_dostuff_componentCss']) {
    
    document['_vaadintheme_dostuff_componentCss'] = true;
  }
